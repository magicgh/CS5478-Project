...
class PyBullet:
    ...
    def control_joints(self, body: str, joints: np.ndarray, target_angles: np.ndarray, speed: float) -> None:
        ...
        self.physics_client.setJointMotorControlArray(
            ...
            positionGains = speed * np.ones(len(joints))
        )
    ...
    def get_body_id_from_name(self, body_name: str) -> int:
    ...
    def reset_joint_state(self, body: str, joint: int, targetValue: float) -> None:
        """Reset the joint state.

        Args:
            body (str): Body unique name.
            joint (int): Joint index in the body.
            targetValue (float): Target angle.
        """
        self.physics_client.resetJointState(bodyUniqueId=self._bodies_idx[body], jointIndex=joint, targetValue=targetValue)
        
    def get_joint_info(self, body: str, joint: int) -> np.ndarray:
        """Get the joint info.
        ...
        Returns:
            np.ndarray: The joint info.
        """
        return self.physics_client.getJointInfo(bodyUniqueId=self._bodies_idx[body], jointIndex=joint)
