...
class MotionPlanAndGraspExp(object):
    def __init__(self, exp_num):
        ...
        self.is_obstacles = 'Without Obstacles'
        ...
        self.cur_exp_num = 0
        self.success_num = 0
        if os.path.exists('exp_results/motion_plan_and_grasp_exp.txt'):
            os.remove('exp_results/motion_plan_and_grasp_exp.txt')
    ...
    def next_state(self):
        ...
        if self.cur_state >= len(self.states):
            ...
            if self.cur_exp_num == self.exp_num:
                ...
                self.save_result()
                self.cur_planner += 1
                if self.cur_planner == len(self.planners):
                    self.cur_planner = 0
                    if self.is_obstacles == 'Without Obstacles':
                        self.is_obstacles = 'With Obstacles'
                        self.add_obstacles()
                    else:
                        return True
            self.reset()
    ...
    def check_success(self):
        ...
        if 0.2 <= lego_pos[0] <= 0.8 and -0.3 <= lego_pos[2] <= 0.3:
            self.success_num += 1
    
    def save_result(self):
        with open('exp_results/motion_plan_and_grasp_exp.txt', 'a') as f:
            f.write(self.planners[self.cur_planner] + ' ' + self.is_obstacles + ': ' + str(self.success_num / self.exp_num) + '\n')
        self.success_num = 0
    ...
    def step(self):
        ...
        if self.state == 1:
            ...
            if len(self.paths) == 2:
                self.paths[0][0].execute(self.paths[0][1], 0.0, True)
        elif self.state == 4:
            self.finger_target = 0.0
        elif self.state == 5:
            if len(self.paths) == 2:
                self.paths[1][0].execute(self.paths[1][1], 0.0, True)
    ...
    def get_path(self, des_pos, des_orn):
        ...
        pb_ompl_interface.set_planner(self.planners[self.cur_planner])
